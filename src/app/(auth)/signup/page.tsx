...
export default function SignupPage() {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    try {
      ...
      if (signinData.error) { setError("Account created! Please sign in."); router.push("/login"); return; }
      router.push("/chat");
      router.refresh();
  ...
  return (
    <div className="bg-vault-900/50 border border-vault-700/40 rounded-2xl p-6 backdrop-blur-sm shadow-2xl">
      <h1 className="font-display text-2xl text-vault-100 mb-1">Create account</h1>
      <p className="text-sm text-vault-400 mb-6">Start building with Vault</p>
      ...
      <p className="text-center text-sm text-vault-500 mt-4">
        Already have an account? <Link href="/login" className="text-vault-300 hover:text-vault-200">Sign in</Link>
      </p>
    </div>
  );
}
