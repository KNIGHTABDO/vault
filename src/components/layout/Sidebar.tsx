...
export function Sidebar({ className = "", allowCollapse = true, onNavigate }: SidebarProps = {}) {
  const router = useRouter();
  const pathname = usePathname();
  const [conversations, setConversations] = useState<Conversation[]>([]);
  const [collapsed, setCollapsed] = useState(false);
  const [email, setEmail] = useState("");
  const isCompact = allowCollapse && collapsed;
  const sidebarWidth = allowCollapse ? (collapsed ? 64 : 260) : 280;
  ...
  async function handleDelete(id: string) {
    const deletingToastId = toast.loading("Deleting chat...");

    try {
      const response = await fetch(`/api/conversations/${id}`, { method: "DELETE" });
      if (!response.ok) {
        let message = "Unable to delete chat.";
        try {
      ...
      if (activeId === id) navigateTo("/chat");
      toast.success("Chat deleted.", { id: deletingToastId });
    } catch (error: any) {
      toast.error(error?.message || "Unable to delete chat.", { id: deletingToastId });
    }
  }
  ...
  return (
    <AnimatePresence>
      <motion.aside
        ...
        className={`h-full bg-vault-950 border-r border-vault-800/60 flex flex-col overflow-hidden relative z-20 ${className}`}
      >
        {/* Header */}
        <div className="p-3 flex items-center justify-between border-b border-vault-800/40">
          <button onClick={() => navigateTo("/chat")} className="flex items-center gap-2 hover:opacity-80 transition-opacity">
            <VaultLogo size="sm" animate={false} />
            {!isCompact && <span className="font-display text-sm text-vault-300">vault</span>}
          </button>
          {allowCollapse && (
        ...
        <nav className="px-2 space-y-0.5">
          {navItems.map((item) => (
            <button key={item.href} onClick={() => navigateTo(item.href)}
        ...
        {!isCompact && (
          <div className="flex-1 overflow-y-auto px-2 mt-3 space-y-3 scrollbar-thin">
            {Object.entries(grouped).map(([label, convs]) => (
              <div key={label}>
                <p className="text-2xs font-semibold uppercase tracking-wider text-vault-600 px-2 mb-1">{label}</p>
                <div className="space-y-0.5">
                  {convs.map(conv => (
                    <div key={conv.id} className="group relative">
                      <button onClick={() => navigateTo(`/chat/${conv.id}`)}
            ...
            <div className="pt-2 mt-2 border-t border-vault-800/40">
              <p className="text-2xs font-semibold uppercase tracking-wider text-vault-600 px-2 mb-1">Info</p>
              <div className="space-y-0.5">
                {infoLinks.map((link) => (
                  <button
                    key={link.href}
                    onClick={() => navigateTo(link.href)}
                    className="w-full text-left px-3 py-2 rounded-lg text-sm text-vault-400 hover:bg-vault-800/50 hover:text-vault-300 transition-all"
                  >
                    {link.label}
                  </button>
                ))}
        ...
        {/* User Menu */}
        <div className={`border-t border-vault-800/40 ${isCompact ? "p-1.5" : "p-2"}`}>
          <div className={isCompact ? "flex justify-center" : ""}>
            <UserMenu email={email} compact={isCompact} />
          </div>
        </div>
      </motion.aside>
    </AnimatePresence>
